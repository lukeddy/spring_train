...
public class CreateClientExample {
    ...
    private static final String CONNECT_STRING="192.168.1.253:2181,192.168.10.159:2181,192.168.1.254:2181";

    public static void main(String[] args) throws Exception {
        CuratorFramework client = null;
        try {
            client = createSimple(CONNECT_STRING);
            client.start();
            client.create().creatingParentsIfNeeded().forPath(PATH, "test".getBytes());
            CloseableUtils.closeQuietly(client);

            client = createWithOptions(CONNECT_STRING, new ExponentialBackoffRetry(1000, 3), 1000, 1000);
            client.start();
